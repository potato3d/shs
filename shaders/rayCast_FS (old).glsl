...
int inOutCastLinear( inout vec4 current, inout vec4 step, sampler2D heightmapIn, sampler2D heightmapOut )
{
	float heightIn;
	float heightOut;
	vec4 s = step;
...
*/
	...
	do
	{
		...
		if( current.z > height/* + 0.2*/ ) // todo: magic constant, should scale instead of add?
		{
			// Draw odd
		...
		do
		{
			...
			if( condition == EXIT )
			{
				...
				// Draw even
			    outCastBinary( current, step, 4, u_hm2 );
				current-= step;
#ifdef testa_layer
				normal.xyz = vec3(flag,1,flag);
			...
			if( current.z > height )
			{
				...
				// TODO: why this is here and there's none in if above
				height = 2.0; // todo: testing, forcing exit of outer loop
				break;
			}
